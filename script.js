/* ==========================================================================
   گالریا — Archive of Objects (v2) interactions
   ========================================================================== */
(() => {
  'use strict';

  const PRODUCTS = [
    { id:'p1', cat:'statue', no:'No. 001', title:'مجسمه «بانوی آرام»', material:'رزین و روکش برنز', price:1850000, seed:'galleria2-p-statue-1', badge:'پرفروش' },
    { id:'p2', cat:'clock',  no:'No. 012', title:'ساعت «مینیمال طلایی»', material:'فلز و شیشه', price:1240000, seed:'galleria2-p-clock-1', badge:null },
    { id:'p3', cat:'art',    no:'No. 024', title:'تابلو «غروب کویر»', material:'بوم و چاپ روغن', price:2100000, seed:'galleria2-p-art-1', badge:'جدید' },
    { id:'p4', cat:'mirror', no:'No. 031', title:'آینه «هاله طلایی»', material:'فریم برنجی دست‌ساز', price:2950000, seed:'galleria2-p-mirror-1', badge:null },
    { id:'p5', cat:'statue', no:'No. 007', title:'مجسمه «پرنده آزادی»', material:'سرامیک لعاب‌دار', price:980000, seed:'galleria2-p-statue-2', badge:null },
    { id:'p6', cat:'clock',  no:'No. 018', title:'ساعت «چوب گردوی کلاسیک»', material:'چوب گردو و فلز', price:1650000, seed:'galleria2-p-clock-2', badge:null },
    { id:'p7', cat:'art',    no:'No. 029', title:'تابلو «انتزاعی آبی»', material:'بوم و آکریلیک', price:1790000, seed:'galleria2-p-art-2', badge:'پرفروش' },
    { id:'p8', cat:'mirror', no:'No. 036', title:'آینه «دایره مدرن»', material:'فریم فلزی مشکی', price:1420000, seed:'galleria2-p-mirror-2', badge:null },
  ];

  const ARRIVALS = [
    { title:'مجسمه «رقص باد»', price:2340000, seed:'galleria2-a-1' },
    { title:'ساعت «برج کوچک»', price:1180000, seed:'galleria2-a-2' },
    { title:'تابلو «باغ ایرانی»', price:2600000, seed:'galleria2-a-3' },
    { title:'آینه «برگ زیتون»', price:1950000, seed:'galleria2-a-4' },
    { title:'مجسمه «سکوت»', price:1420000, seed:'galleria2-a-5' },
    { title:'ساعت «آفتابگردان»', price:1080000, seed:'galleria2-a-6' },
  ];
  ...
  function productCard(p){
    ...
    el.innerHTML = `
      <div class="index-media">
        ...
        <button class="index-quick" aria-label="نمای سریع ${p.title}">${eyeIcon}</button>
      </div>
      <div class="index-info">
        <div class="index-row-top">
          <span class="index-no">${p.no}</span>
          ${p.badge ? `<span class="index-badge">${p.badge}</span>` : ''}
        </div>
        <h3 class="index-title">${p.title}</h3>
        <span class="index-material">${p.material}</span>
        <div class="index-foot">
          <span class="index-price">${money(p.price)}</span>
          <button class="index-add" data-id="${p.id}" aria-label="افزودن ${p.title} به سبد خرید">${cartIcon}</button>
        </div>
      </div>`;
    return el;
  }
  ...
  grid.addEventListener('click', e => {
    const btn = e.target.closest('.index-add');
    if(!btn) return;
  ...
  function closeMenu(){
    menuToggle.classList.remove('open'); mainNav.classList.remove('open'); menuToggle.setAttribute('aria-expanded','false');
    if(!cartDrawer.classList.contains('open') && !document.getElementById('searchPanel').classList.contains('open')) overlay.classList.remove('show');
  }
  ...
  function resetTimer(){ clearInterval(testiTimer); testiTimer = setInterval(() => goToSlide(current + 1), 6000); }
  ...
  document.getElementById('newsletterForm').addEventListener('submit', e => {
    ...
    if(email.value.trim()){
      showToast('عضویت شما در <strong>خبرنامه گالریا</strong> ثبت شد 🌿');
      email.value = '';
    }
  });
